...
def get_folds(total_data, n_splits=3):
    ...
    actions_label_sbj = actions_label_sbj[shuff_inds]# random shuff all
    
    # cross-actions this one only consider action but no use
    folds = {}
    ...
    for num_fold, (train_index, test_index) in enumerate(StratifiedKFold(n_splits=n_splits).split(actions_list, actions_labels)):
        folds[num_fold] = {'indexes': test_index.tolist(), 
                           ...
                           } # only save the test index of fold
 
        
    # cross-subject  but no use
    folds_subject = {}
    ...
    # cross-subjects-folds  but no use
    ...
    # 3d PostureNet evaluation # this one
