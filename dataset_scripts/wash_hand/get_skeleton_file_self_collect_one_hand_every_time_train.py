...
with mp_hands.Hands(model_complexity=0, min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
    ...
    for action in os.listdir(path_data + 'HandWashDataset_self_train_every_time/'):
        v_i = 0
        for video in os.listdir(path_data + 'HandWashDataset_self_train_every_time/' + action):
            v_i = v_i + 1
            # For webcam input:
            cap = cv2.VideoCapture(path_data + 'HandWashDataset_self_train_every_time/' + action + '/' + video)
            video_name = video.split('.')[0]
            subject = video_name[-1]
            path_joint = path_data + 'handwash/HandWashDataset_self_one_hand_every_time/' + str(v_i) + '/' + action + '/'
