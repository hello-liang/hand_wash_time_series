...
def load_data(data_format = 'common_minimal'):
    with open(os.path.join("/datasets/handwash/HandWashDataset_self_one_hand/2/Step1/joint.txt")) as f:
        skels = f.read().splitlines()
    skels = np.array([list(map(float, l.split())) for l in skels])
    print(skels.shape)
#    skels=skels[10:50,:]#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    #!!!!!!!!here change the length of data to check
...
def get_folds(total_data, n_splits=3):
    ...
    actions_list = np.array([  total_data ])
